...
import {
  fetchAboutPageThunk,
  updateAboutPageThunk,
} from "../../store/dataSlice";
import { emptyAboutPage } from "../../services/mappers";
...
export const AdminAboutPage = () => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchAboutPageThunk())
      .unwrap()
      .then((data) => {
        if (!cancelled) setForm({ ...emptyAboutPage, ...data });
      })
      .catch((err) => {
        if (!cancelled) {
          setError(err || t("failedToLoadAbout"));
          setForm(emptyAboutPage);
        }
      })
      .finally(() => {
  ...
  const handleSubmit = async (event) => {
    ...
    setSaveMessage("");
    try {
      await dispatch(updateAboutPageThunk(form)).unwrap();
      setSaveMessage(t("aboutSaved"));
      setTimeout(() => setSaveMessage(""), 3000);
    } catch (err) {
      setError(err || t("failedToSaveAbout"));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className={cn("max-w-3xl")}>
      ...
      <p className={cn("text-sm text-gray-500 mb-6")}>{t("aboutPageHint")}</p>
      ...
      >
        <Input
          ...
          placeholder={t("aboutTitlePlaceholderEn")}
          required
        />
        <Input
          label={t("titleAr")}
          ...
          placeholder={t("aboutTitlePlaceholderAr")}
          required
        />
        ...
        <div className={cn("flex flex-col gap-2")}>
          ...
          <textarea
            ...
            placeholder={t("aboutContentPlaceholderEn")}
            className={cn(
        ...
        <div className={cn("flex flex-col gap-2")}>
          ...
          <textarea
            ...
            placeholder={t("aboutContentPlaceholderAr")}
            className={cn(
              "w-full min-h-[160px] px-4 py-3 border border-gray-300 rounded-lg outline-none focus:border-[#e60023] text-sm resize-y",
            )}
          />
